...
# Auto-exclude ONLY the Docker bridge subnet (eth0's own subnet).
# WARP's routing table routes 172.0.0.0/6 through CloudflareWARP, which
# swallows Docker bridge traffic and breaks port-forwarding reply routing.
# We do NOT exclude all RFC-1918 — user LAN ranges (e.g. 192.168.x.x)
# may be intentionally reachable via the WARP Zero Trust tunnel.
_eth0_subnet=$(ip -4 -o route show dev eth0 scope link 2>/dev/null | awk '{print $1}' | head -1)
if [ -n "$_eth0_subnet" ]; then
    log "  Auto-excluding Docker bridge subnet: $_eth0_subnet"
    warp-cli --accept-tos tunnel ip add-range "$_eth0_subnet" || true
fi
...
# WARP's table 65743 may route the Docker bridge subnet through CloudflareWARP,
# which causes reply packets from Dante to Docker bridge clients to be misrouted.
# Add a high-priority ip rule ONLY for the Docker bridge subnet (eth0).
# Do NOT add rules for all RFC-1918 — user LAN subnets are intentionally routed
# through the WARP tunnel in Zero Trust setups.
if [ -n "$_eth0_subnet" ]; then
    log "Adding ip rule for Docker bridge $_eth0_subnet → main table (priority 100)..."
    ip rule add to "$_eth0_subnet" table main priority 100 2>/dev/null || true
fi
